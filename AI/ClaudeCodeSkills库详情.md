...
| `skill-vetter`                  | 安全审计外部 Skill——安装前审核 SKILL.md，6 轮检测（注入/凭据/RCE/供应链/typosquatting/社会工程）                                                       |
| `skill-sync-repo`               | 同步 Skill 到个人库                                                                                                              |
| `vision`                        | 火山引擎豆包视觉模型识图，vision.js 调用豆包 API，模型无原生识图能力时使用                                                                               |
| `xlsx`                          | 电子表格创建、编辑、数据分析（含公式、格式化、可视化）                                                                                                |
| `openclaw-skills-pdf-translate` | **PDF 英文→中文翻译**（LobeHub 安装）：pdfplumber 提取文本 → AI 逐章节翻译 → Markdown + PDF 双格式输出（weasyprint CJK 排版）。触发："翻译PDF""pdf translate" |
| `disk-cleaner-skills`           | **Windows/macOS 磁盘清理专家**：5 层检测引擎，三色安全分级（🟢安全/🟡需确认/🔴谨慎），200+ 受保护应用，交互式 HTML 报告。触发："磁盘满了""清理电脑""释放空间""空间不够"                |
...
| MCP 名称                   | 功能说明                                                                                                                | 安装状态 |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------- | ---- |
| `undoom-uninstaller-mcp` | Windows 程序卸载 + 残留清理：list_programs / uninstall_program / force_remove_program / clean_residues，对标系统"设置→应用→卸载"但多了残留扫描 | 已装全局 |
...
| `openclaw-skills-pdf-translate`✅✅✅ | PDF 英文→中文翻译：pdfplumber提取文本→AI逐章节翻译→Markdown+PDF双格式输出（weasyprint CJK排版） | 全局 + 个人库 |
...
| `skill-vetter`                  | 安全审计外部 Skill——安装前 6 轮检测                                                                                               | 已装全局 + 个人库 |
...
| `obsidian-contentList` | 笔记段落式描述转为层级有序/无序列表，保留代码块/表格/callout，与 obsidian-titiled 协作 | 项目级 + 个人库 |
...
| Skill 名称 | 功能说明 | 安装状态 |
|-----------|---------|---------|
| `disk-cleaner-skills` | Windows/macOS 磁盘清理：5 层检测引擎 + 三色分级 + 交互式 HTML 报告，扫描全程只读，删除通过安全服务器执行 | 全局 + 个人库 |
...
| MCP 名称 | 功能说明 | 安装状态 |
|---------|---------|---------|
| `undoom-uninstaller-mcp` | Windows 程序卸载 + 残留清理 | 全局 + 个人库 |
