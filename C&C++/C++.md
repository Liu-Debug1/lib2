...
只要一个类被设计成"可以被继承"，它的析构函数就应该写 `virtual`：
```cpp
class Vehicle {
public:
    virtual ~Vehicle() { }  // 必须！
...
// 如果不加 virtual，下面就是未定义行为：
Vehicle* v = new Car();
delete v;  // 灾难——只调了 Vehicle 的析构，Car 的部分没释放
```

原理：`delete` 基类指针时，如果析构函数不是 virtual，编译器只看指针的静态类型（Vehicle），不会调用子类析构。加 `virtual` 后，运行时会查到实际类型（Car），正确调用 Car 析构 → Vehicle 析构，逐层释放。
