...
**2.命名空间的作用**
- 增加标识符的使用率，减少命名产生的冲突
- 充当<font color="#000000">容器</font>，作用于不同的作用域阿斯顿阿斯顿
- 只能写在**全局区域**
...
| 函数                                  | 核心行为                        | 空白字符处理            | 常用场景与要点                                                 |
| ----------------------------------- | --------------------------- | ----------------- | ------------------------------------------------------- |
| `std::cin >> value`                 | 按变量类型解析一个格式化数据              | 跳过前导空白，读到空白停止     | 读取 `int`、`double`、`char`、单词字符串。`std::string` 中的空格会截断输入。 |
| `std::cin.get()`                    | 读取一个字符                      | **不跳过**空格、Tab 和换行 | 要保留空白时使用；无参形式返回 `int`，以便表示 EOF。                         |
| `std::cin.getline(buffer, count)`   | 读取一行到 `char[]`              | 保留行内空格，丢弃结尾换行     | 旧式 C 风格缓冲区；`count` 必须包含字符串结尾的 `\0` 空间。                  |
| `std::getline(std::cin, line)`      | 读取一行到 `std::string`         | 保留行内空格，丢弃结尾换行     | **推荐**的整行文本输入方式。                                        |
| `std::cin.ignore(count, delimiter)` | 丢弃至多 `count` 个字符，或在遇到分隔符时停止 | 用于清理已读取数据后的残留字符   | `>>` 后接 `getline` 前，丢弃残留的 `\n`。                         |
| `std::cin.peek()`                   | 查看下一个字符但不取走                 | 不改变缓冲区            | 用于根据下一个字符决定后续解析逻辑。                                      |
| `std::cin.fail()`                   | 查询流是否处于失败状态                 | 无                 | 类型不匹配等失败后，后续读取都会失效。                                     |
| `std::cin.clear()`                  | 清除错误状态标志                    | 不清理缓冲区            | 必须先 `clear()`，再用 `ignore()` 丢弃错误输入。                     |

##### (3) 统一示例

统一示例：依次演示格式化输入、失败恢复、`getline` 的换行清理、`cin.getline()`、`peek()` 和 `get()`。
```cpp
#include <iostream>
#include <limits>
#include <string>
...
/*
 * 功能：演示 std::cin 常用输入函数及其缓冲区处理方式。
 * 参数：无。
 * 返回值：输入流正常结束时返回 0。
 * 修改输入数据：无；仅读取标准输入流。
 * 前置条件与边界：从控制台交互运行；整数输入错误时会提示并要求重新输入。
 * 内存与并发：仅使用栈对象；不适用于中断上下文；标准输入流不保证多线程并发访问安全。
 */
int main()
{
    int age{};
    ...
    while (true) {
        ...
        std::cout << "年龄必须是整数，请重新输入。\n";
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    }

    // >> 成功后，换行仍留在缓冲区；getline 前需要先丢弃它。
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    ...
    std::cout << "请输入一个字符（空格也可以）：";
    const int nextChar = std::cin.peek();
    if (nextChar != '\n' && nextChar != std::char_traits<char>::eof()) {
        const char firstChar = static_cast<char>(std::cin.get());
        std::cout << "peek() 看到并由 get() 读出的字符是：" << firstChar << '\n';
    }

    std::cout << "年龄：" << age << "，姓名：" << fullName
              << "，地址：" << address << '\n';
    return 0;
}
```
...
> [!warning] 注意
> `std::cin.clear()` 只清除错误状态，**不清除缓冲区**。处理无效输入时必须先 `clear()`，再 `ignore()`；实际项目中优先使用 `std::numeric_limits<std::streamsize>::max()`，不要写固定的 `100` 或 `10000`。


### 1.2 变量声明与初始化

#### 1.2.1 C++ 允许在 `for` 循环内部声明循环变量：
```cpp
// C: 变量必须在函数开头声明
int i;
for (i = 0; i < 10; i++) { }
...
**`enum class` 是 C++ 中推荐的枚举写法，解决了传统 C enum 的两大核心缺陷——作用域污染和类型不安全。**

三种枚举的核心区别：

| 特性 | C enum | C++ enum | `enum class` |
|------|--------|----------|--------------|
| 非枚举值赋值 | ✅ 允许 | ❌ 禁止 | ❌ 禁止 |
| 跨类型赋值 | ✅ 允许 | ❌ 禁止 | ❌ 禁止 |
| 作用域 | 暴露到全局 | 暴露到全局，但可通过 `枚举名::值` 限定访问 | 完全限定，**必须**通过 `枚举名::值` |
| 跨类型比较 | ✅ 允许 | ✅ 允许 | ❌ 编译报错 |
| 推荐使用 | 兼容旧代码 | 过渡写法 | 🥇 **默认首选** |
...
1. **允许非枚举值赋值，也允许跨枚举类型赋值**——类型检查几乎不存在
   ```cpp
   enum WEEK today = 3;    // 正确：int 直接赋给 enum
   today = CIRCLE;         // 正确：跨类型赋值也不拦
   ```

2. **枚举元素暴露到外层作用域，容易命名冲突**
   遵循就近原则，外层同名符号会被遮蔽：
   ```cpp
   enum OTHER { RECT };  // error C2365: "RECT": 重定义；以前的定义是"枚举数"
   int RECT = 12;        // 同上，enum 值已经占了 RECT 这个名字
   ```

3. **不同类型的枚举值可以直接比较**——编译器不认为这是错误
   ```cpp
   if (CIRCLE == MON) {
...
> [!warning] C enum 的根本问题
> C enum 本质上就是一组**有名字的整数常量**。编译器不区分”SHAPE 类型”和”WEEK 类型”，也不区分”枚举值”和”裸整数”。这在大型工程中极易引入隐蔽的类型错误。

#### 1.7.2 C++ 中的传统 enum

C++ 传统 enum 对 C enum 做了部分收紧，但作用域问题仍在：

1. **仅允许赋值合法的枚举值**——类型检查比 C 严格
   ```cpp
   enum WEEK today = 3;    // 错误 error C2440: 无法从"int"转换为"main::WEEK"
   today = CIRCLE;         // 错误 error C2440: 无法从"main::SHAPE"转换为"main::WEEK"
   ```

2. **同名枚举值仍会冲突**，但可通过 `枚举名::枚举元素` 限定访问
   ```cpp
   enum OTHER { RECT };  // error C2365: "RECT": 重定义
   OTHER::RECT;          // 正确访问方式——用作用域解析符限定
   ```

3. **不同类型的枚举值依旧可以直接比较**——这个缺陷没有被修复
   ```cpp
   if (CIRCLE == MON) cout << "oh.yes";  // 仍然编译通过
   ```

#### 1.7.3 C++ 的 `enum class`（强枚举类型）

`enum class` 彻底解决了前述所有问题。
...
1. **限定作用域**——枚举值不暴露到全局，**必须**通过 `枚举名::枚举元素` 访问
   ```cpp
   cout << static_cast<int>(SHAPE::RECT) << endl;  // 输出 1
   // RECT  // ❌ 未定义——enum class 不会把这个名字放到全局
   ```
   `enum class` **不会隐式转换为 int**。确实需要打印底层值时，必须显式 `static_cast<int>()`——这正是强类型安全的体现。

2. **强类型安全**——不相关的枚举类型之间禁止比较，编译期直接拦截
   ```cpp
   if (SHAPE::CIRCLE == WEEK::MON)
       // error C2676: "==": "main::SHAPE" 不定义该运算符
   ```

> [!tip] 工程建议：默认使用 `enum class`
> 除非你在维护 C 遗留代码或需要隐式转 int 的场景（如数组索引），否则新代码一律用 `enum class`。它把错误从”运行时踩坑”提前到”编译期拦截”，这正是 C++ 类型系统的核心价值。

#### 1.7.4 技巧：用 switch 快速列出所有枚举成员

`enum class` 配合编译器自动补全，可以秒写 switch 分支：

1. 在 `switch()` 中输入枚举类型名（如 `SHAPE`），回车
2. 编译器自动生成所有 `case` 分支
3. 将 `SHAPE` 替换为实际的枚举变量名

```cpp
switch (shape) {          // 第 3 步：改成真正的变量名
case SHAPE::CIRCLE:       // 编译器自动生成的 case 列表
    break;
case SHAPE::RECT:
    break;
case SHAPE::LINE:
...
**与之前知识的关联：**
- `enum class` 的"用类型系统在编译期拦截错误”与 [1.7 struct 与 class](#17-struct-与-class) 中的访问修饰符逻辑一致——都是把约束写进类型，让编译器替你检查
- 作用域限定（`SHAPE::RECT`）与 `namespace` 的设计意图相通——隔离命名，各管各的

**嵌入式/线控底盘场景：**
- CAN 帧 ID、错误码、状态机状态——这三类数据用 `enum class` 定义，能避免把 `CAN_ID` 和 `ERROR_CODE` 混用
- `enum class` 默认底层类型是 `int`，如果枚举值不多，可以用 `enum class X : uint8_t` 节省内存
- FreeRTOS 的任务状态（`eRunning`、`eReady` 等）在官方实现中就是传统 enum，阅读源码时理解 C enum / C++ enum / enum class 三者的区别有助于理解 HAL 层的类型选择
...
### 1.10 动态内存分配
#### 1.10.1 `new/delete`和`malloc/free`
...
这里释放的内存是指针指向地址的内存，但是指针变量本身还存在，而且依旧指向这个地址。
处于安全考虑， 还需要将指针指向置空
```cpp
指针变量 = nullptr
```
...
#### 1.14.2 为什么需要函数重载？
1. 🥕 避免函数名爆炸——同一功能只用同一个名字，不同参数由编译器区分
2. 🥕 构造函数天生同名，重载让实例化更灵活
3. 🥕 运算符重载本质就是函数重载，让 `+` 也能拼接字符串
...
4. **移动构造**：移动：直接"抢"走资源，原对象变空壳
	...
	- 🛡️ unique_ptr 只能移动不能拷贝：独占资源的所有权转移
```cpp
 class Point_c {
	  int x, y; // 默认 private
  public:
...
> [!NOTE] 如果这个类会被继承，析构函数还需要加 `virtual`，详见 [1.26 规则二](#规则二基类析构函数必须是-virtual)。
...
`explicit` 禁止构造函数被用于隐式类型转换。加在构造函数声明前即可：
...
没有 `explicit` 时，编译器会做你不期望的隐式转换：
- `’a’`（97）→ 自动转成 `int`，再调构造函数
- `10` → 自动转成 `MyString(10)`
- `"maye"` → 自动转成 `MyString("maye")`

以下代码展示了显式 vs 隐式构造的区别：
```cpp
MyString s(20);             // ✅ 显式构造，合理
MyString ss = 10;           // ❌ 隐式转换
MyString ts = MyString(10); // ✅ 显式匿名对象构造，合理
MyString s1 = 'a';          // ❌ 隐式转换（'a'→97→MyString(97)）
MyString s2("磨课");        // ✅ 显式构造
MyString s3 = "maye";       // ❌ 隐式转换
```

#### 1.16.3 使用规则

- **原则：所有构造函数前默认加 `explicit`**，当确实需要隐式转换时再解除
- **原因**：隐式转换导致的逻辑错误编译零警告，极难排查——别给编译器猜你意图的机会
- **例外**：有意识利用隐式转换时（如 `std::string s = "hello";`），可以不加
...
`using` 有两大用途：**类型别名**（替代 `typedef`）和**名字引入**（拉名字进当前作用域）。

#### 1.17.1 类型别名 — `typedef` 的现代化替
...
`using` 比 `typedef` 强的地方在于**可以和模板配合**——`typedef` 做不到：

```cpp
template<typename T>
class CircularBuffer {
public:
    using value_type = T;  // 外部用 CircularBuffer<int>::value_type 拿到 int
};
```

#### 1.17.2 名字引入 — 把名字"拉进"当前作用域

**场景 1：解除派生类对基类重载函数的遮蔽**

```cpp
class Base {
public:
    void calibrate(int id) { }
    void calibrate(double tolerance) { }
};

class Derived : public Base {
public:
    using Base::calibrate;          // 把 Base 所有 calibrate 重载"请下来"
    void calibrate(const char* name) { }  // 再追加自己的新版本
};

Derived d;
d.calibrate(5);       // 调用 Base::calibrate(int)
d.calibrate(0.01);    // 调用 Base::calibrate(double)
d.calibrate("motor"); // 调用 Derived::calibrate(const char*)
```

没有 `using Base::calibrate` 的话，`int` 和 `double` 版本全被派生类的 `calibrate(const char*)` 遮蔽了。

**场景 2：`using namespace` 引入命名空间**

```cpp
using namespace std;  // 省的每次写 std::cout
```

> [!warning] 嵌入式项目里尽量别用 `using namespace std`
> 它会污染全局作用域，容易引发命名冲突。用 `std::cout`、`std::array` 显式前缀更安全。

| 用法 | 嵌入式里用不用 |
|------|-------------|
| `using 别名 = 类型` | 用到，替代 `typedef`，更现代 |
| `using Base::func` | 偶尔，写继承体系时用 |
| `using namespace std` | 尽量别用，污染命名空间 |
...
- **C++11**：类内声明（加 `static`），类外定义初始化（加 `类名::`）
- **C++17**：可在类内直接 `inline static` 初始化，无需类外定义
...
public:
    Stu() : age(0) { classSize++; }
    static int classSize;               // 类内声明
};
int Stu::classSize = 0;                 // 类外定义（必须！）

// C++17 写法
class Stu {
    inline static int classSize = 0;    // 一行搞定
};
```
...
```cpp
Stu zc;
cout << zc.classSize << endl;           // 通过对象访问
cout << Stu::classSize << endl;         // 通过类名访问（仅静态成员可这样）
```
...
// 访问
Stu zc;
int num = zc.getClassSize();            // 通过对象
int num = Stu::getClassSize();          // 通过类名
```

#### 1.18.3 注意事项

- **静态函数只能访问静态成员**——没有 `this` 指针，无法访问非静态成员变量/函数
- **类外定义不要加 `static`**——只在类内声明时用。类外写 `static` 会被当成全局静态变量，而不是类成员的定义：
  ```cpp
  int Counter::count = 0;               // ✅ 正确
  // static Counter::count = 0;         // ❌ 编译器不认识
  ```
- **初始化位置**：C++11 在类外的 `.cpp` 中，C++17 可直接在类内 `inline static`
...
**注意点 1：const 成员函数中的 this**

在 `const` 成员函数里，`this` 的类型会变成 `const ClassName* const`——既不能改指针指向，也不能通过 `this` 修改成员变量。这是 C++ 保证 const 正确性的底层机制。

**注意点 2：链式调用必须返回引用，不能返回值**

以上面的 `Counter` 为例，如果 `increment()` 返回的是 `Counter`（值返回）而非 `Counter&`（引用返回）：

```cpp
Counter increment() {   // ❌ 值返回
    this->count++;
    return *this;       // 返回的是当前对象的临时副本
}
```

那么 `c.increment().increment().print()` 的行为完全不同：
- 第一次 `increment()`：原对象 `c.count` 从 5 变为 6，但返回的是 **临时副本**（副本 count=6）
- 第二次 `increment()`：修改的是 **临时副本** 的 count（从 6 变为 7），临时副本在表达式结束后销毁
- 最终 `print()`：打印的是原对象 `c` 的 count，值只有 6（只被第一次 increment 改过）

> 链式调用的本质是"在同一个对象上连续操作"，必须返回引用。值返回的语义是"给你一个副本，随便玩"，副本的修改与原对象无关。
...
#### 1.21.4 现代 C++ 的替代方案：std::function
...
Lambda 就是一个**即写即用、用完就扔的小函数**，不用单独起名字。
...
// 捕获外部变量——把外面的东西拿进来用
int count = 0;
auto inc = [&]() { count++; };  // [&] 引用捕获，能直接改 count
```
...
- **没有继承时**：Car 和 Truck 各自写一遍 speed、position、start()、stop()，改一处漏一处
- **有了继承**：公共部分提到 Vehicle 基类，Car 和 Truck 只写各自特有部分
...
- 继承最重要的工程价值
- 不同类型对象继承自同一基类 → 通过基类指针/引用统一操作，不需要知道具体是哪个子类
...
##### (2) 规则二：基类析构函数必须是 virtual

- 🎯 **核心原则**：只要一个类被设计成"可以被继承"，析构函数就写 `virtual`
- **为什么**：若父类析构非虚，通过父类指针 `delete` 子类对象时，子类析构不会被调用，子类资源泄漏
...
| 从不通过父类指针 `delete` 子类 | 栈上对象、智能指针直接持有子类类型 |
| 父类析构不需要做什么事          | 依赖这点是侥幸心理，不推荐     |

**工程规则**：****
- 类里有任何虚函数 → 析构也应该是虚的（多态的类大概率会被多态地使用）
- 不打算被继承 → 不加 `virtual`（省掉 vtable 开销）

简记：**不打算被继承 → 不加 virtual；打算被继承 → 析构必虚。**

##### (3) 规则三：区分三种继承方式

| 继承方式 | 基类public→子类 | 基类protected→子类 | 基类private→子类 | 含义 |
|:---------|:---------------|:------------------|:----------------|:-----|
| public | public | protected | 不可见 | "is-a"，最常用 |
| protected | protected | protected | 不可见 | "is-implemented-in-terms-of"，罕见 |
| private | private | private | 不可见 | "is-implemented-in-terms-of"，用组合更好 |

> [!tip] private 成员在子类中*依然存在*，但无法访问（可通过父类公开接口间接访问）

🎯 **如何选择访问级别**：
- 需要被外界访问 → `public`
- 只在当前类中访问 → `private`
- 只在当前类和子类中访问 → `protected`
- 一般情况无特殊要求 → 均选 `public` 继承

---
##### (4) 规则四：不想被继承就标记 final
...
🌳 类型兼容性原则：**子类对象可以当作父类对象来用**，编译器默认允许，无需显式转换。

5 种具体体现——本质上都是向上转型（upcasting，把子类当成父类看）：
...
- 🌳 类型兼容性是 C++ 面向对象的**地基**——没有它，继承只剩代码复用（最无聊也最危险的用法）
...
// 有类型兼容性——一个函数搞定所有子类
void doSomething(Father& f) { f.action(); }
```

- **类型兼容性 + 虚函数 = 多态**：
  - 类型兼容性：让"用基类接口操作子类对象"在语法上可行
  - 虚函数：让它在语义上正确（调的是子类函数，不是基类的）

#### 1.26.3 条核心规则

🎯 **规则一：传指针或引用保留多态，传值会切片（slicing）**
...
```
- `byValue(son)` 编译不报错——类型兼容性允许——但子类部分被静默切除（C++ 最安静的 bug 之一）

🎯 **规则二：基类析构必须是 virtual**

- 因果链：类型兼容性 → 基类指针持有子类对象 → 迟早要释放 → 析构必须是虚的
```cpp
class Father {
public:
    virtual ~Father() {}   // 少这个就是定时炸弹
};

Father* p = new Child(10);
delete p;   // 非虚析构 → Child 资源泄漏
```
...
for (auto* p : family) {
    p->ShowFather();    // 同一段代码，不同子类不同行为
}
```
- 没有类型兼容性，`vector<Father*>` 只能装 `Father`——多态编程退回 C 时代

#### 1.26.4 同名遮蔽（name hiding）——继承里最容易静默出错的坑

- **规则**：子类中任何与父类同名的成员，都会遮蔽父类的那个成员——不管类型、参数是否相同
- **原因**：编译器按名字查找，在子类找到就停，父类版本全部不可见
...
- **永远用指针或引用持有子类对象**——传值是切片的邀请函
- **基类必须有虚析构**——别寄望于"没人会通过基类指针 delete"
- **针对基类接口编程**——新增子类时，依赖基类接口的代码一行不改
- **不要在子类定义和父类同名的变量**——函数需扩展时用 `using Base::funcName;` 拉下来再加新版本

### 1.27 static 在继承中的行为

#### 1.27.1 核心规则：static 成员只有一份，不属于任何对象
...
- 只读：试图写入触发段错误
- 函数指针指向的就是这片区域
- 嵌入式对应：STM32 Flash 区域，掉电不丢失

**2. 常量区（.rodata）**——只读数据

```cpp
const char* p = "hello";   // "hello" 在 .rodata，p 本身在栈上
const int MAX = 100;       // 全局 const 放在 .rodata
```

- 存放：字符串字面量、`const` 全局变量
- 试图修改 `"hello"[0] = 'H'` → 崩溃
- 嵌入式：放 Flash，节省 RAM
...
```cpp
int g_count = 10;          // 有非零初值
static int s_id = 1;
```

- 初值在程序加载时写好
- 嵌入式：启动代码从 Flash 拷贝初值到 RAM

**4. BSS 段**——未初始化或初始化为 0 的全局/静态变量

```cpp
int g_buffer[1024];        // 没初始化
static int s_counter;      // 初值为 0
```

- 程序启动时自动清零
- **不占可执行文件体积**（只记录"需要多大空间"，不存初值）
- 嵌入式：启动代码 `memset` 清空 BSS 区域
...
- `new`/`malloc` 取内存，`delete`/`free` 归还
- 忘释放 → 内存泄漏；释放后还继续用 → 悬空指针
...
- 后进先出（LIFO），`}` 即隐式弹栈
- **嵌入式注意**：MCU 栈通常只有几 KB（STM32 默认约 1KB），大数组放栈上有溢出风险——数据覆盖到堆/全局区，变量被莫名其妙改、程序跑飞
...
1. 没有 `virtual`（静态绑定）：编译器只看指针的**静态类型**（声明时写的类型），不管实际指向什么。
```
Father* p = &son;
p->show();
  ↓
...
2. 有 `virtual`（动态绑定）：运行时通过虚函数表查到**实际类型**，调对应的函数。
```
Father* p = &son;
p->show();
  ↓
...
> [!NOTE] 多态的三要素
> 1. **要有继承关系**——子类从父类派生
> 2. **要用父类指针/引用指向子类对象**——类型兼容性原则（见 [[#1.27 类型兼容性原则|1.27 节]]）是多态的前提，不用指针/引用会切片
...
**1. 统一接口操作不同对象——写一次，适配所有子类**

```cpp
// 没有多态：每加一种设备就要改主循环
if (device == MOTOR_A)      motorA_start();
else if (device == MOTOR_B)  motorB_start();
else if (device == MOTOR_C)  motorC_start();   // 每次加新设备都要改

// 有多态：主循环永远不用改
Actuator* devices[] = {&motorA, &motorB, &motorC};
for (auto d : devices) {
    d->start();   // 不管多少种设备，这一行就够了
}
```

**2. 支持开闭原则——对扩展开放，对修改关闭**

- 加新传感器类型 → 只写新派生类，框架代码一行不动
- 没有多态 → 框架里到处加 `else if`

**3. 代码直接表达设计意图**

- `Actuator` 有 `emergencyStop()` 虚函数
- 刹车 → "立即制动"、转向 → "回正"、油门 → "断油"
- 代码结构 = 系统结构，看代码就知道设计
...
public:
    ...
    virtual ~Actuator() {}        // 基类析构必须 virtual（见 1.26 节规则二）
};
...
虚函数开销：每个对象多一个 vtable 指针（4~8 字节），每次调用多一次间接跳转。

- **大多数场景够用**——现代编译器优化能力强，间接跳转开销通常可忽略
- **ISR 里避免**——中断要求确定执行时间，间接跳转引入不确定性
- **极度频繁的小函数**——可用模板（编译期多态）替代 virtual

> 一句话：先用虚函数把架构写对，profiler 找到瓶颈再优化。别提前优化。
...
### 1.33 C++ 强制类型转换 vs C 风格转换

#### 1.33.1 C 风格转换的问题

 C 语言只有一种转换写法 `(Type)expr`，但它干了三件完全不同的事：
```cpp
int a = (int)3.14;        // 数值转换（合理）
(char*)&a;                // 去掉 const（危险）
*(int*)&f;                // 重新解释位模式（非常危险）
```
一个 `(Type)` 语法，编译器不检查，读者猜不出意图，`grep` 也搜不到它——因为括号语法太普遍了。
#### 1.33.2 C 风格转换在 C++ 中的真实行为

C 风格 `(Type)expr` 在 C++ 里会按顺序尝试：
1. `const_cast` + `static_cast`
2. `static_cast` + `const_cast`
3. `reinterpret_cast` + `const_cast`
最后一步是真正危险的地方——你本来只想做数值转换，但如果不小心传了不相关的类型，C 风格会默默退化到 `reinterpret_cast`，编译器不给你任何警告。

********#### 三、C++ 四种命名转换

```cpp
static_cast      = 编译期类型规则检查 + 不做运行时安全验证
dynamic_cast     = 运行时真实类型检查，主要用于多态继承
const_cast       = 专门处理 const / volatile，能转不代表能安全改
reinterpret_cast = 按地址或位模式强行重新解释，类型安全最低
```
##### (1) static_cast —— 最常用的

相关类型之间的转换，编译器会检查是否合法：
```text
类型类别是否允许
继承关系是否存在
转换路径是否明确
...
```cpp
double pi = 3.14159;
int n = static_cast<int>(pi);     // 窄化转换，明确告诉编译器"我就要丢掉小数"

void* ptr = &n;
int* ip = static_cast<int*>(ptr); // void* → int*，恢复原始类型

// 基类指针向上转换（安全的）
Derived d;
Base* bp = static_cast<Base*>(&d); // OK，编译器检查继承关系
```

##### (2) dynamic_cast —— 运行时安全检查
沿继承树向下转换，成功时放回指向地址的指针，失败时返回 `nullptr`（指针）或抛异常（引用）：
```cpp
Base* bp = getSomeObject();
Derived* dp = dynamic_cast<Derived*>(bp);
if (dp) {
    dp->derivedMethod();  // 安全，确认了真是 Derived
}
```

> [!NOTE] 通俗类比
> `static_cast` 向下转型 = 你指着穿便衣的人说"你是警察吧，上车"，他说是你就信了——万一他不是呢？
> `dynamic_cast` 向下转型 = 你要求他出示警官证，验完是真的才让他上车——`nullptr` 就是"没证件，拒绝"。
> 所以题目中说"需要运行时检查是否安全"——就是要查证件的场景，必须用 `dynamic_cast`。

##### (3) const_cast —— 唯一合理用途是兼容老 C API

> [!NOTE] 通俗类比
> `const` 就像快递箱上贴的"易碎品，勿压"标签。`static_cast` 是普通搬运工——能帮你把箱子搬到不同货架（类型转换），但**不能撕标签**。`const_cast` 是唯一被授权撕标签的人。
> 你写 `static_cast<char*>(str)` → 编译器报错："箱子上贴着易碎品标签，我没权限撕"。
> 你写 `const_cast<char*>(str)` → 编译器放行，但你自己要确保那个函数不会真的压坏箱子。

```cpp
// C 库函数不保证不修改字符串，但实际不修改
void legacyLog(char* msg);  // 老 C 库，没写 const
const char* msg = "hello";
legacyLog(const_cast<char*>(msg));  // OK：我们知道 legacyLog 不会改它
```
> [!caution] 绝对不要用来修改原本就是 const 的对象——结果是未定义的：
> ```cpp
> const int x = 10;
> const_cast<int&>(x) = 20;  // 未定义行为！x 可能被编译器放在只读区
> ```

##### (4) reinterpret_cast —— 最危险的一个
直接重新解释内存中的位模式，不改变任何二进制位：
```cpp
// 嵌入式最常见的用法：硬件寄存器映射
#define UART1_BASE 0x40011000
UART_Reg* uart = reinterpret_cast<UART_Reg*>(UART1_BASE);
// 把 int 的位模式当作 float 读——这不是数值转换！
int bits = 0x40490FDB;
float f = reinterpret_cast<float&>(bits);  // 结果可能不是你想的那样
```
每写一个 `reinterpret_cast`，就是在和编译器的类型系统对赌。
...
// C 风格——你猜我想干嘛？
int b = (int)d;          // 数值转换？还是位模式？分不清
int c = (int)&a;         // 地址转整数——这又是什么意图？

// C++ 风格——意图写在脸上
int b = static_cast<int>(d);                       // 我在做数值转换
int c = reinterpret_cast<int>(&a);                 // 我在把地址转整数
void* p = const_cast<void*>(static_cast<const void*>(&a));  // 我去掉了 const
```
...
| 场景                    | 用哪个                 |
| --------------------- | ------------------- |
| 浮点转整型、类型提升/窄化         | `static_cast`       |
| void* 恢复为原始类型指针       | `static_cast`       |
| 基类指针安全转为派生类指针         | `dynamic_cast`      |
| 调用没有 const 正确性的老 C 函数 | `const_cast`        |
| 硬件地址映射、位模式重新解释        | `reinterpret_cast`  |
| 任何 C 风格 `(Type)x`     | 原则上不用——替换成上面对应的命名转换 |

> [!tip] 要点
> - Prefer C++-style casts to C-style casts——编译器替你审查，grep 替你定位，意图写在脸上
> - `reinterpret_cast` 应该被集中封装在少数几个底层访问函数里，而不是散布在各处
> - 代码里出现的 `(Type)x` 应该和裸 `new`/`delete` 一样——你最好有个很好的理由
> - 嵌入式里寄存器映射是最合理的 `reinterpret_cast` 使用场景，但封装好了只写一次
...
作用：
	1.给名称建立**别名**
	2.把某个/整个**作用域**中的名字引入当前作用域


| 写法                         | 作用                                         |
| -------------------------- | ------------------------------------------ |
| `using NewType = OldType;` | 类型别名                                       |
| `using std::vector;`       | 引入一个指定名称，之后用到vector只写vector，不用写std::vector |
| `using namespace std;`     | 引入整个命名空间，不建议在工程全局/头文件使用                    |
...
// 遍历字符串的每个字符
int i = 0;
for (const auto& x : s) cout << x << "(" << i++ << ")  "; // 1(0) 2(1) 3(2) 2(3) 1(4) 3(5)
cout << endl;
