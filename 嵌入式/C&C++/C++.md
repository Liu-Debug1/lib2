...
本质就是编译器帮你生成一个匿名类+`operator()`，零额外开销。

> [!warning] 线控底盘/车规嵌入式里基本不用
> MISRA C++ / AUTOSAR 规范限制 lambda，原因：捕获列表容易隐藏副作用，静态分析工具不好过，出了问题不好追溯。实际项目代码仍是 C 风格的状态机+函数指针为主。但在**测试框架（Google Test）和调试工具**里偶尔用到，知道就行。
...
| 转换                 | 用途             | 一句话                       |
| ------------------ | -------------- | ------------------------- |
| `static_cast`      | 相关类型间的显式转换     | "这两个类型有关系，编译器帮我做标准转换"     |
| `dynamic_cast`     | 继承体系中安全向下转换    | "运行时检查这个基类指针是否真的指向派生类"    |
| `const_cast`       | 去除或添加 const 属性 | "我要改变 const 属性——几乎总是坏主意"  |
| `reinterpret_cast` | 无关类型间的位模式重新解释  | "我知道自己在干什么，把这块内存当成另一个类型读" |
...
> [!warning] 嵌入式注意
> `dynamic_cast` 依赖 RTTI（运行时类型信息），会增大代码体积和运行时开销。微控制器上通常关闭 RTTI（`-fno-rtti`），此时 `dynamic_cast` 不可用。如果代码里需要 `dynamic_cast`，通常说明设计有问题——优先考虑用虚函数替代。
